  ...
  if (!formData.name.trim()) {
    return (
      <div className="rounded-lg border-2 border-dashed border-border-main p-6 text-center h-full flex items-center justify-center min-h-[400px]">
          <p className="font-sans text-sm text-text-secondary">Начните настраивать эксперта, чтобы увидеть превью...</p>
      </div>
    );
  }
    ...
    <div className="space-y-6 rounded-lg border border-border-main bg-bg-surface p-6 shadow-lg h-full">
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-x-6 gap-y-3 font-sans text-sm text-text-secondary">
            <div><p>Конструктивность</p><StatBar value={formData.character.constructiveness} /></div>
            <div><p>Конформизм</p><StatBar value={formData.character.conformism} /></div>
            <div><p>Убежденность</p><StatBar value={formData.character.conviction} /></div>
            <div><p>Открытость к данным</p><StatBar value={formData.character.opennessToData} /></div>
            <div className="col-span-2"><p>Креативность (t°)</p><StatBar value={formData.character.temperature} max={2.0} /></div>
        </div>
