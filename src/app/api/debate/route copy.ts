...
function buildSystemPrompt(expert: ConfiguredExpert, allExperts: ConfiguredExpert[], debateGoal: string, brief: string): string {
    ...
    // ── 7. ПРИКАЗЫ К ИСПОЛНЕНИЮ v10.0 ──
    p += `\n## OUTPUT COMMANDS (ПРИКАЗЫ К ИСПОЛНЕНИЮ)\n` +
         `Твоя миссия — максимально аутентично отыграть свою роль эксперта для достижения цели (CUSTOM MISSION). Твоя задача — не просто выдать информацию, а **быть** своим персонажем. Не трать время на приветствия, сразу переходи к сути.\n` +
         `1.  **ГЛАВНЫЙ ПРИНЦИП: СЛЕДУЙ СВОЕМУ ХАРАКТЕРУ.** Твои действия (атаковать, поддерживать, предлагать, сомневаться) полностью зависят от твоих параметров в секции CHARACTER. Это твой главный закон, который важнее всего остального.
             •   **Если ты деструктивен (низкая Конструктивность):** Ищи уязвимости, риски и слабые места в аргументах других. Твоя цель — критика.
             •   **Если ты конструктивист (высокая Конструктивность):** Ищи сильные стороны в чужих идеях и развивай их. Твоя цель — улучшение.
             •   **Если ты нонконформист (низкий Конформизм):** Иди против течения. Ставь под сомнение то, с чем все согласились.
             •   **Если ты конформист (высокий Конформизм):** Ищи точки соприкосновения и согласия с другими.
             Ты **НЕ ОБЯЗАН** всегда атаковать. Ты обязан быть верным своему профилю.\n` +
         `2.  **РОЛЕВАЯ ИДЕНТИФИКАЦИЯ:** Твоё имя — **«${expert.name}»**. Другие эксперты в этой дискуссии: ${otherExpertsNames || 'нет других'}. Не путай себя с ними.\n` +
         `3.  **ИСКУССТВО ДИАЛОГА:** Естественно взаимодействуй с другими. Если уместно, обращайся к ним по имени, чтобы усилить свою мысль («Иван, это хорошая мысль, но...»). Не каждое твоё сообщение обязано быть прямой репликой кому-то.\n` +
         `4.  **СТИЛЬ И КРАТКОСТЬ (БРЕВИТИ, СУКА!):** Живая речь. Без списков и отчетов. **Максимум 3-4 предложения.** Без компромиссов.\n`;
         `5.  **ЧИСТОТА ПЕРСОНАЖА:** Ты всегда пишешь от своего имени, Ты не имеешь права писать от имени другого собеседника.Пользователь должен четко понимать, что ты - это единственный персонаж .\n`;
    return p;
}
...
export async function POST(req: Request) {
    ...
    const { discussionId, brief, debateGoal, selectedExperts, history } = body;
    const model = body.model ?? 'gpt-4.1-mini'; 
    const saveRun = body.saveRun ?? true; 
    ...
    const stream = new ReadableStream({
        async start(ctrl) {
            ...
            try {
                ...
                // Итерируемся по КАЖДОМУ выбранному эксперту для одного раунда
                for (const ex of selectedExperts) {
                    ...
                    if (modelName.startsWith('deepseek')) {
                        apiClient = deepseek;
                        console.log(`[DEBATE LOG] Expert: ${ex.name} is using DEEPSEEK model: ${modelName}`);
                    } else {
                        apiClient = openai;
                        console.log(`[DEBATE LOG] Expert: ${ex.name} is using OPENAI model: ${modelName}`);
                    }

                    const truncatedHistory = currentHistory.slice(-MAX_MSGS);
    
                    // Вызываем обновленную функцию с брифом
                ...
                } // ✅✅✅ ВОТ ЗДЕСЬ, СКОРЕЕ ВСЕГО, ПРОПАЛА СКОБКА, ЗАКРЫВАЮЩАЯ ЦИКЛ `for` ✅✅✅

                // Сохранение в Firestore сработает после цикла, когда вся история раунда будет собрана
                if (saveRun) {
                    try {
                        await addDoc(
                            collection(db, 'discussions', discussionId, 'runs'),
                            {
                                team: selectedExperts.map(e => ({ id: e.id, name: e.name })),
                                transcript: currentHistory,
                                createdAt: new Date(),
                            }
                        );
                        console.log('Debate run saved to Firestore for discussion:', discussionId);
                    } catch (firestoreError) {
                        console.error('Error saving debate run to Firestore:', firestoreError);
                        send({ type: 'error', message: 'Failed to save debate run to Firestore.' });
                    }
                }
        ...
        cancel() {
            console.log('Stream was cancelled by the consumer.');
        }
    });
